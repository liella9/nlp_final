...
def pipeline_train(train, test, lim_unigram):
    ...
    for instance in train.instances:
        head = instance['Headline']
        body_id = instance['Body ID']
        X_refuting = max(refuting_features(head, train.bodies[body_id]))
        X_polarity = max(polarity_features(head, train.bodies[body_id]))
        ...
        feat_vec = np.squeeze(np.c_[head_tf, body_tf, tfidf_cos, X_refuting, X_polarity])
...
def pipeline_test(test, bow_vectorizer, tfreq_vectorizer, tfidf_vectorizer):
    ...
    for instance in test.instances:
        head = instance['Headline']
        body_id = instance['Body ID']
        X_refuting = max(refuting_features(head, test.bodies[body_id]))
        X_polarity = max(polarity_features(head, test.bodies[body_id]))
        if head not in heads_track:
